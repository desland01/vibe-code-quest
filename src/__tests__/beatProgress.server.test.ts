...
describe('validateBeatStateWrite (server, registry-aware)', () => {
  ...
  it('rejects landmark with no registered sequence', () => {
    expect(validateBeatStateWrite('git', 'branches-as-isolation', state())).toMatchObject({ ok: false, status: 400 });
  });
...
describe('resolveProgressWrite (registry-keyed routing — security boundary)', () => {
  ...
  it('rejects a non-beat landmark claiming kind beat-sequence', () => {
    expect(resolveProgressWrite('git', 'branches-as-isolation', state())).toMatchObject({ path: 'reject', status: 400 });
  });

  it('keeps ordinary legacy state on non-beat landmarks on the legacy path', () => {
    expect(resolveProgressWrite('git', 'branches-as-isolation', { notes: 'x', completed: false })).toMatchObject({ path: 'legacy' });
    expect(resolveProgressWrite('databases', 'sql', { anything: 1 })).toMatchObject({ path: 'legacy' });
  });

  it('leaves v1 legacy behavior untouched for unknown non-beat identifiers', () => {
