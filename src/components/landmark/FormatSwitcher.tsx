...
export function FormatSwitcher({
  ...
  const labels: Record<LandmarkFormat, string> = {
    overview: 'Overview',
    lesson: playLabel ? 'Play' : 'Lesson',
    quiz: 'Quiz',
  };
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    const nextIndex = event.key === 'Home'
      ? 0
      : event.key === 'End'
        ? landmarkFormats.length - 1
        : (index + (event.key === 'ArrowRight' ? 1 : -1) + landmarkFormats.length) % landmarkFormats.length;
    ...
    <div className="format-switcher" role="group" aria-label="Landmark format">
      {landmarkFormats.map((value, index) => (
        <button
          ...
          aria-current={format === value ? 'true' : undefined}
